...
public class GenericHtmlExtractor {
    ...
    public Result extract(byte[] htmlBytes, String baseUrl) {
        ...
        doc.select("script,style,noscript,iframe,svg,canvas,video,audio,form,header,footer,nav,aside").remove();
        doc.select("[class~=(?i)(ad|ads|advert|banner|promo|sponsor|cookie|consent|subscribe|newsletter|share|social|breadcrumbs|login|signup)]").remove();
    ...
    private String normalize(String s) {
        return s.replaceAll("\\s+", " ").trim()
            .replaceAll("(?i)\\b(\\d{1,2}[.:]\\d{2}" +      // время hh:mm
                "|\\d{4}-\\d{2}-\\d{2}" +          // ISO дата
                "|\\d{2}[./]\\d{2}[./]\\d{4}" +    // дата dd.mm.yyyy
                "|\\d+\\s+(minutes?|hours?|days?)\\s+ago)" + // "3 hours ago"
                "\\b", " "); }
